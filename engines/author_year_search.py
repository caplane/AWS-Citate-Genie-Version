...
class AuthorDateEngine:
    ...
    def search(
        ...
        Returns:
            Best matching SourceComponents, or None if not found
        """
        if year == "n.d.":
    ...
    def _search_claude(
        ...
        try:
            ...
            # Build SourceComponents from Claude's guess
    ...
    def _search_gpt4o(
        ...
        api_key = os.environ.get('OPENAI_API_KEY', '').strip().lstrip('=')
        if not api_key:
...
Only respond with valid JSON, no other text."""
            ...
            # Build SourceComponents from GPT-4o's guess
            from models import CitationType
    ...
    def search_multiple(
    ...
    ) -> Dict[Tuple[str, str], Optional[SourceComponents]]:
        """
        ...
        Returns:
            Dict mapping (author, year) to SourceComponents (or None if not found)
        """
        results = {}
        total = len(citations)
