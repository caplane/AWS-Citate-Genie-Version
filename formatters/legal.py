...
class BluebookFormatter(BaseFormatter):
    ...
    def _format_interview(self, m: SourceComponents) -> str:
        ...
        if m.interviewee:
            parts.append(f"Interview with {m.interviewee}")
        elif m.interviewer:
            parts.append(f"Interview by {m.interviewer}")
        else:
            parts.append("Interview")
        
        if m.interviewer and m.interviewee:
    ...
    def _format_case(self, m: SourceComponents) -> str:
        """
    ...
    def _format_case_short(self, m: SourceComponents) -> str:
        """
    ...
    def _format_other(self, m: SourceComponents) -> str:
    ...
    def _format_general_short(self, m: SourceComponents) -> str:
        ...
        if m.authors:
            ...
            parts.append(self._get_last_name(m.interviewee) + " interview,")
        elif m.sender:
...
class OSCOLAFormatter(BaseFormatter):
    ...
    def _format_interview(self, m: SourceComponents) -> str:
        ...
        if m.interviewer and m.interviewee:
            parts.append(f"({m.interviewer}, interviewer)")
    ...
    def _format_general_short(self, m: SourceComponents) -> str:
        ...
        if m.authors:
            ...
            # Fallback for interviews
            parts.append(self._get_last_name(m.interviewee))
        elif m.sender:
            # Fallback for letters
            parts.append(self._get_last_name(m.sender))
        elif m.title:
            # Fallback for URLs/other
            words = m.title.split()[:3]
