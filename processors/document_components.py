"""
processors/document_components.py
...
class SourceComponentsCache:
    ...
    def get(self, citation_text: str) -> Optional[SourceComponents]:
        """
        ...
        Returns:
            SourceComponents if found in cache, None otherwise
        """
        ...
        if hash_key in self._cache:
            ...
            return SourceComponents.from_dict(data.get('metadata', {}))
        
        print(f"[MetadataCache] Cache MISS for hash {hash_key}: {citation_text[:40]}...")
        return None
    
    def set(self, citation_text: str, metadata: SourceComponents) -> None:
        """
        Store metadata in the cache.
        
        Args:
            citation_text: The original citation text
            metadata: The resolved SourceComponents
        """
        hash_key = hash_citation_text(citation_text)
    ...
    @classmethod
    def from_xml_string(cls, xml_string: str) -> 'SourceComponentsCache':
        """
        ...
        Returns:
            SourceComponentsCache instance
        """
        cache = cls()
...
def load_cache_from_docx(file_bytes: bytes) -> SourceComponentsCache:
    """
    ...
    Returns:
        SourceComponentsCache (may be empty if no cache found)
    """
    try:
            ...
            for path in possible_paths:
                if path in names:
                    ...
                    if '<citategenie' in content or CITATEGENIE_NS in content:
                        ...
                        return SourceComponentsCache.from_xml_string(content)
            
            print("[DocumentMetadata] No existing cache found in document")
            return SourceComponentsCache()
...
def save_cache_to_docx(file_bytes: bytes, cache: SourceComponentsCache) -> bytes:
    """
    ...
    Args:
        file_bytes: The document as bytes
        cache: The SourceComponentsCache to embed
