...
Version History:
    ...
    2025-12-06 12:25 V3.3: Added resolve_place fallback in _book_dict_to_components
...
2. Legal extraction uses superlegal.extract_components() for cache + CourtListener API
...
ARCHITECTURE:
- Wrapper classes convert superlegal.py/books.py dicts → SourceComponents
...
def classify_with_ai(query: str, context: str = "") -> Tuple[CitationType, Optional[SourceComponents]]:
    """
    ...
    Returns:
        Tuple of (CitationType, optional SourceComponents)
    """
...
def _score_author_position(result: SourceComponents, query: str) -> float:
    """
...
def _legal_dict_to_components(data: dict, raw_source: str) -> Optional[SourceComponents]:
    """Convert superlegal.py extract_components() dict to SourceComponents."""
    if not data:
        return None
...
def parse_existing_citation(query: str) -> Optional[SourceComponents]:
    """
    ...
    Returns SourceComponents if parsing succeeds, None otherwise.
    """
...
def _parse_journal_citation(query: str) -> Optional[SourceComponents]:
    """
...
def _parse_book_citation(query: str) -> Optional[SourceComponents]:
    """
...
def _parse_newspaper_citation(query: str) -> Optional[SourceComponents]:
    """
...
def _route_legal(query: str) -> Optional[SourceComponents]:
    """
    ...
    """
    try:
        data = superlegal.extract_components(query)
        if data and (data.get('case_name') or data.get('citation')):
            return _legal_dict_to_components(data, query)
...
def _route_book(query: str) -> Optional[SourceComponents]:
    """
    ...
    """
    try:
        results = books.extract_components(query)
        if results and len(results) > 0:
            return _book_dict_to_components(results[0], query)
    except Exception as e:
...
def _route_journal(query: str, gist: str = "") -> Optional[SourceComponents]:
    """
...
def route_citation(query: str, style: str = "chicago", context: str = "", components_cache=None) -> Tuple[Optional[SourceComponents], str]:
    """
    Main entry point: route query to appropriate engine and format result.
    
    Returns: (SourceComponents, formatted_citation_string)
    ...
    Args:
        ...
        components_cache: Optional SourceComponentsCache for embedded document cache
...
def get_parenthetical_options(
    ...
    style: str = "APA 7", 
    limit: int = 5
) -> List[Tuple[Optional[SourceComponents], str]]:
    """
    ...
    Returns:
        List of (SourceComponents, formatted_string) tuples.
...
def get_parenthetical_components(
    citation_text: str, 
    limit: int = 5,
    context: str = ""
    ...
    Returns:
        List of SourceComponents objects (unformatted).
    """
    try:
        from engines.ai_lookup import lookup_parenthetical_citation_options
        ...
        print(f"[UnifiedRouter] Error in get_parenthetical_components: {e}")
        return []
...
def get_citation(query: str, style: str = "chicago", context: str = "", components_cache=None) -> Tuple[Optional[SourceComponents], str]:
    """Alias for route_citation() - backward compatibility."""
    return route_citation(query, style, context, components_cache)
...
def search_citation(query: str) -> List[dict]:
    ...
    # Try books
    try:
        book_results = books.extract_components(query)
